...
while ( have_posts() ) : the_post(); 
    ...
    $ii++;
    echo '<article class="excerpt excerpt-'.$ii. $_excerpt_text .wow().'">';
        ...
        echo '<header>';
            ...
            if( _hui('home_sticky_s') && is_home() && is_sticky()){
		...
		if(get_post_meta($post->ID,'pay_switch',true) == 1 && _hui('buy_btm')) echo'<small><a href="'.get_permalink().'#pay-content" title="'.get_the_title().'" target="_blank">立即购买</small>';
