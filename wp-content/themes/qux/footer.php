...
<?php 
	if( _hui('kefu') ){ 
		...
		if( _hui('kefu_m') && wp_is_mobile() ){
			$kefuorder = _hui('kefu_m_px');
		}else{
			$kefuorder = _hui('kefu_px');
		}
		if( $kefuorder && is_array($kefuorder)){
			foreach ($kefuorder as $key => $value) {
				switch ($value) {
					...
					case '6':
						if( (is_single()||is_page()) && comments_open() && !_hui('kill_comment_s') ) $kefuhtml .= '<li><a href="javascript:(scrollTo(\'#comments\',-15));"><i class="fa fa-comments"></i><span>'._hui('kefu_comment_tip_m').'</span></a>'.(_hui('kefu_comment_tip')?'<h6>'. _hui('kefu_comment_tip') .'<i></i></h6>':'').'</li>';
						break;
...
<?php  
    ...
    $fullimage = 0;
    if( ((is_single() && _hui('full_image')) || (is_page() && _hui('page_full_image'))) && !wp_is_mobile() ){
		$fullimage = 1;
	}
