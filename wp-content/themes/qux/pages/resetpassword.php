...
case 'resetpass' :
	...
	if ( is_wp_error($user) ) {
		if ( $user->get_error_code() === 'expired_key' ){
			wp_redirect( mo_get_user_rp() . '?act=lostpassword&error=expiredkey' );
		}
		else{
			wp_redirect( mo_get_user_rp() . '?act=lostpassword&error=invalidkey' );
		}
		exit;
	}
	...
	if ( ( ! $errors->get_error_code() ) && isset( $_POST['pass1'] ) && !empty( $_POST['pass1'] ) ) {
		...
		wp_redirect( mo_get_user_rp() . '?act=success' );
		exit;
	}
...
<div class="content-wrap">
	<div class="content resetpass">
		...
		if( $classactive1 ){ 
			if( $errors !== true ){
		?>
		<form action="<?php echo esc_url( mo_get_user_rp() . '?act=lostpassword' ); ?>" method="post">
			<?php errormsg($errors); ?>
			<h3>填写用户名或邮箱：</h3>
