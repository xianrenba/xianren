<?php
// About author module
function mo_author_info_module(){
	global $post;
	$pid = get_the_ID();
	...
	$wechat = $author_info->wechat_pay;
	$alipay_email = $author_info->um_alipay_email;
	$author_home = $author_info->user_url;
?>
<div class="sg-author clr">
	<div class="img"><?php echo um_get_avatar( $author , '90' , um_get_avatar_type($author) ); ?></div>
	<div class="sg-author-info">
		<div class="word">
			<div class="wordname"><i class="fa fa-user"></i><?php the_author_posts_link(); ?></div>
			<div class="authordes"><?php the_author_meta('description'); ?></div>
			<div class="authorsocial">
			<?php echo um_follow_button($author_info->ID,'span'); ?>
			<?php if(!empty($donate) || !empty($wechat)){ ?>
			<span class="social-icon-wrap"><a class="as-img as-donate" href="#" title="<?php _e('赞助作者','um'); ?>"><i class="fa fa-coffee"></i><?php _e('赞助作者','um'); ?>
				<div id="as-donate-qr" class="as-qr"><img src="<?php echo $donate; ?>" title="<?php _e('支付宝扫一扫赞助作者','um'); ?>" /></div>
			</a><?php echo um_alipay_post_gather($alipay_email,1,1); ?></span>
			<?php } ?>
			<?php if(!empty($author_home)){ ?>
			<span class="social-icon-wrap"><a class="as-img as-home" href="<?php echo $author_home; ?>" title="<?php _e('作者主页','um'); ?>"><i class="fa fa-home"></i></a></span>
			<?php } ?>
			<?php if(!empty($sinawb)){ ?>
			...
			<span class="social-icon-wrap"><a class="as-img as-weixin" href="#" id="as-weixin-a" title="<?php _e('微信','um'); ?>"><i class="fa fa-weixin"></i>
				<div id="as-weixin-qr" class="as-qr"><img src="<?php echo $weixin; ?>" title="<?php _e('微信扫描二维码加我为好友','um'); ?>" /></div>
			</a></span>		
			<?php } ?>
			<?php if(!empty($qq)){ ?>
