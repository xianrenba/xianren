...
class widget_ui_sticky extends WP_Widget {
	...
	function __construct(){
		parent::__construct( 'widget_ui_sticky', 'qux 置顶推荐', array( 'classname' => 'widget_ui_posts' ) );
	}
...
?>
<li><a<?php echo _post_target_blank() ?> href="<?php the_permalink(); ?>"><?php if( $img ){echo '<span class="thumbnail">'; echo _get_post_thumbnail(); echo '</span>'; }else{$img = '';} ?><span class="text"><?php the_title(); ?><?php echo get_the_subtitle() ?></span><span class="muted"><?php the_time('Y-m-d');?></span><span class="muted"><?php echo '评论(', comments_number('', '1', '%'), ')'; ?></span></a></li>
<?php
