...
class widget_ui_readers extends WP_Widget {
	...
	function __construct(){
		parent::__construct( 'widget_ui_readers', 'qux 活跃读者', array( 'classname' => 'widget_ui_readers' ) );
	}
...
function dtheme_readers($outer,$timer,$limit,$addlink){
	...
	foreach ($comments as $comment) {
		...
		if($id===0){
			$user_avatar = get_avatar( $user_author=$comment->comment_author, $size );
		}else{
		...
		$html .= '<li><a title="['.$comment->comment_author.'] 近期点评'. $comment->cnt .'次" target="_blank"'.$c_urllink.'>'
		.$user_avatar.
		'</a></li>';
	}
	echo $html;
