...
function product_shortcode($atts, $content = null){
	...
	if(!empty($id)) {
        ...
        $money = product_smallest_price($id);
	    ...
	    $html .= '<div class="embed-product"><img class="product-img" src="'.post_thumbnail_src( get_post($id)).'">';
	    $html .= '<div class="product-info"><h4><a href="'.$href.'">'.$title.'</a></h4><div class="price">';
	    $html .= $money[4] ? '<del>'.$preice.'<span>'.$money[0].'</span></del>&nbsp;&nbsp;&nbsp;<span>'.$money[2].'</span><small>(限时优惠)</small>' : $preice.'<span>'.$money[0].'</span>';
	    $html .= '</div><div class="post-meta">&nbsp;数量：<span>'.$amount.'</span>&nbsp;&nbsp;&nbsp;销量：<span>'.$sales.'</span></div>';
	    $html .= '<a class="btn btn-success btn-buy" href="'.$href.'"><i class="fa fa-shopping-cart"></i>'.$content.'</a></div></div>';	
	}else{
		$html .= '<button type="button" class="btn btn-product btn-'.$size.'">'.$content.'</button>';
	}
	return $html;
}
