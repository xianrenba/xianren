...
function mo_slider( $id='slider' ){
    ...
    foreach ($sort as $key => $value) {
        if( _hui($id.'_src_'.$value) && _hui($id.'_href_'.$value) && _hui($id.'_title_'.$value) ){
            ...
            $inner .= '<div class="item'.(!$i?' active':'').'"><a'.( _hui($id.'_blank_'.$value) ? ' target="_blank"' : '' ).' href="'._hui($id.'_href_'.$value).'"><img src="'._hui($id.'_src_'.$value).'"><span class="carousel-caption">'._hui($id.'_title_'.$value).'</span></a></div>';
