		...
		<ul v-else v-cloak>
			<li v-for="item in data" class="pd10 pos-r">
				<div v-if="item.type == 1" class="pos-r fs14 msg-item gray">
					<span v-html="item.users"></span> 在<span v-html="item.post_title"></span>中回复了你的评论 <time :data-timeago="item.date" class="timeago fs12 dot">{{item.date}}</time>
					<div><p class="msg-content" v-html="item.comment"></p></div>
				</div>
				<div v-if="item.type == 3" class="pos-r fs14 msg-item gray">
					<span v-html="item.users"></span> 评论了你的<span v-html="item.post_title"></span><time :data-timeago="item.date" class="timeago fs12 dot">{{item.date}}</time>
					<div><p class="msg-content" v-html="item.comment"></p></div>
				</div>
				...
				<div v-if="item.type == 32" class="pos-r fs14 msg-item gray">
					<span v-html="item.users"></span> 购买了你的付费内容： <span v-html="item.post_title"></span> <time :data-timeago="item.date" class="timeago fs12 dot">{{item.date}}</time>
				</div>
				...
				<div v-if="item.type == 34" class="pos-r fs14 msg-item gray">
					<span v-html="item.users"></span> 购买了你的积分阅读内容： <span v-html="item.post_title"></span> <time :data-timeago="item.date" class="timeago fs12 dot">{{item.date}}</time>
				</div>
				<div v-if="item.type == 10" class="pos-r fs14 msg-item gray">
					<span v-html="item.users"></span> <b class="green">喜欢</b> 你在 <span v-html="item.post_title"></span> 中的评论。<time :data-timeago="item.date" class="timeago fs12 dot">{{item.date}}</time>
					<div><p class="msg-content" v-html="item.comment"></p></div>
				</div>
				<div v-if="item.type == 8" class="pos-r fs14 msg-item gray">
					<span v-html="item.users"></span> <b class="red">不喜欢</b> 你在 <span v-html="item.post_title"></span> 中的评论。<time :data-timeago="item.date" class="timeago fs12 dot">{{item.date}}</time>
					<div><p class="msg-content" v-html="item.comment"></p></div>
				</div>
				<div v-if="item.type == 35" class="pos-r fs14 msg-item gray">
					<span v-html="item.users"></span> 对你的文章进行了打赏 ：<span v-html="item.post_title"></span> <time :data-timeago="item.date" class="timeago fs12 dot">{{item.date}}</time>
					<div><p class="msg-content" v-html="item.comment"></p></div>
				</div>
				<div v-if="item.type == 19" class="pos-r fs14 msg-item gray">
					<span v-html="item.users"></span> 回复了你的帖子 ：<span v-html="item.post_title"></span> <time :data-timeago="item.date" class="timeago fs12 dot">{{item.date}}</time>
				</div>
				<div v-if="item.type == 20" class="pos-r fs14 msg-item gray">
					<span v-html="item.users"></span> 在话题 <span v-html="item.post_title"></span> 中提到了你！<time :data-timeago="item.date" class="timeago fs12 dot">{{item.date}}</time>
				</div>
				...
				<div v-if="item.type == 11" class="pos-r fs14 msg-item gray">
					<span v-html="item.users"></span> 关注了你 <time :data-timeago="item.date" class="timeago fs12 dot">{{item.date}}</time>
				</div>
				<div v-if="item.type == 15" class="pos-r fs14 msg-item gray">
					<span v-html="item.users"></span> 取消了对你的关注 <time :data-timeago="item.date" class="timeago fs12 dot">{{item.date}}</time>
				</div>
				<div v-if="item.type == 14" class="pos-r fs14 msg-item gray">
					<span v-html="item.users"></span> 对你的<b>积分</b>进行了变更，变更原因： <time :data-timeago="item.date" class="timeago fs12 dot">{{item.date}}</time>
				...
				<div v-if="item.type == 37" class="pos-r fs14 msg-item gray">
					<span v-html="item.users"></span> 对你的<b>余额</b>进行了变更，变更原因： <time :data-timeago="item.date" class="timeago fs12 dot">{{item.date}}</time>
					<div><p class="msg-content" v-html="item.comment"></p></div>
				</div>
				<div v-if="item.type == 12" class="pos-r fs14 msg-item gray">
					<span v-html="item.users"></span> 给你发来了私信： <a href="<?php echo home_url('/directmessage'); ?>">查看 ❯</a> <time :data-timeago="item.date" class="timeago fs12 dot">{{item.date}}</time>
				</div>
				...
				<div v-if="item.type == 44" class="pos-r fs14 msg-item gray">
					<span><?php echo get_avatar($user_id,48); ?></span> <span v-html="item.users"></span> 报名参加了你的活动 <span v-html="item.post_title"></span> <time :data-timeago="item.date" class="timeago fs12 dot">{{item.date}}</time>
				</div>
