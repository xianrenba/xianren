				...
				<div class="pd10 lin">
					...
					<div class="widget-user-total mar10-b b-t pd10-t mar20-t">
						<a href="<?php echo zrz_get_user_page_url($user_id,'posts'); ?>" class="fd">
							...
							<p class="gray">文章</p>
						</a><?php if(zrz_get_display_settings('labs_show')){ ?><a href="<?php echo zrz_get_user_page_url($user_id,'labs'); ?>" class="fd">
							<span><?php echo count_user_posts($user_id, 'labs' ); ?></span>
							<p class="gray"><?php echo zrz_custom_name('labs_name'); ?></p>
                        </a><?php } ?><a href="<?php echo zrz_get_user_page_url($user_id,'follow'); ?>" class="fd">
					...
					<?php
                        if((is_user_logged_in() && $c_user_id == $user_id) || current_user_can('delete_users')){
    ...
    public function widget( $args, $instance ) {
        ...
        if(!empty($users)){
            foreach ($users as $user) {
                ...
                $user_html .= '
                        <li class="pd10 pos-r b-t fs12 clearfix">
            ...
            $user_html .='<div class="pd10 t-c gray b-t">没有财富数据</div>';
        }
        $user_html .='</ul>';
        echo $user_html;
        if($fixed){
   ...
   <?php
    ...
    public function widget( $args, $instance ) {
        ...
        if ( $the_query->have_posts() ) :
            ...
            $html .= '<div class="b-t pd10 pos-r">
                    <a href="'.get_permalink().'" rel="bookmark">
                    '.$thumb.'<h2 class="'.($thumb ? '' : 'mar0').'">'.get_the_title().'</h2></a></div>';
             endwhile;
            wp_reset_postdata();
         else :
            $html .= '<div class="pd10 b-t t-c gray">没有热门文章</div>';
         endif;
         echo $html;
         if($fixed){
             echo '</div>';
...
class Zrz_Rand_Posts extends WP_Widget {
    ...
    public function widget( $args, $instance ) {
        ...
        echo $before_title.$instance[ 'title' ].$after_title;
        $html = '<div class="rand-post b-t pd10">';
        $query = new WP_Query('posts_per_page=1&ignore_sticky_posts=1&orderby=rand');
        if ( $query->have_posts() ) {
           while ( $query->have_posts() ) {
               $query->the_post();
                $thumb = zrz_get_post_thumb() ? '<img src="'.zrz_get_thumb(zrz_get_post_thumb(),270,$instance[ 'height' ]).'" class=" rand-post-img mar10-b" />' : '';
                $html .= '<a href="'.get_the_permalink().'">'.$thumb.'<h2>'.get_the_title().'</h2></a>';
           }
           wp_reset_postdata();
        }else{
           $html .= '<div class="pd10 t-c gray">没有随机文章</div>';
        }
    ...
    public function widget( $args, $instance ) {
        ...
        if ( $query->have_posts() ) {
           while ( $query->have_posts() ) {
               ...
               $html .= '<div class="clearfix b-t pd10"><a class="hot-shop-img fl" href="'.get_the_permalink().'">'.$thumb.'</a>
    ...
    public function widget( $args, $instance ) {
        ...
        if($data){
            foreach ($data as $val) {
                ...
                $html .= '<li class="pd10 b-t cleafix">
   ...
   <?php
    ...
    public function widget( $args, $instance ) {
        ...
        if ( $the_query->have_posts() ) :
            ...
            $html .= '<div class="b-t pd10">
                    '.get_avatar(get_the_author_meta('ID') ,24).'
                    <h2 class="widget-post-h2"><a href="'.get_permalink().'">'.get_the_title().'</a></h2></div>';
             endwhile;
            wp_reset_postdata();
         else :
            $html .= '<div class="pd10 b-t t-c gray">没有帖子</div>';
         endif;
         echo $html;
         if($fixed){
             echo '</div>';
   ...
   <?php
    ...
    public function widget( $args, $instance ) {
        ...
        <div id="newest-comments" data-hide="'.$instance[ 'hide_author' ].'" data-number="'.$instance[ 'newest_comments_nub' ].'">
                <ul v-if="comments.length > 0" v-cloak>
                    <li class="b-t pd10" v-for="comment in comments">
                        <div class="clearfix">
