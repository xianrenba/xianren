...
class zrz_get_user_data{
    ...
    public static function get_avatar($type = false){
        ...
        if($avatar_set == 'default'){
            ...
            if($avatar_host == 0){
                return isset(self::$user_open['weixin_avatar_new']) ? self::$user_open['weixin_avatar_new'] : '';
            }
...
function zrz_unbind_social(){
    ...
    if(get_the_author_meta('user_email', $user_id) == '' && !isMobile($user_data->user_login) ){
...
function zrz_get_new_msg_count(){
    ...
    if($count_new){
        print json_encode(array('status'=>200,'count'=>$count_new));
        exit;
    }else{
        print json_encode(array('status'=>401,'count'=>0));
        exit;
    }
...
function zrz_get_dmsg_user_list(){
    ...
    $all_sql = "msg_type=13 AND user_id=".$user_id;
...
function zrz_dmsg_delete(){
    ...
    $resout = $wpdb->update(
        $table_name,
        array('msg_read'=>3),
        array('msg_key'=>$user_id,'user_id'=>$current_id)
    );
        ...
        if($user_id && $lv){
			...
			if(strpos($lv,'vip') !== false){
				...
				update_user_meta($user_id,'zrz_vip_time',array(
					...
					'end'=>$lv_setting['time'] == 0 ? 0 : date("Y-m-d H:i:s",strtotime('+'.$lv_setting['time'].' day'))
				));
			}
