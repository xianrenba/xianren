...
//每天凌晨任务数据重置
add_action( 'wp', 'zrz_rec_setup_daily_schedule');
add_action( 'clear_zrz_rec_daily_event', 'zrz_rec_do_this_daily_rst');

function zrz_rec_setup_daily_schedule(){
    if (!wp_next_scheduled( 'clear_zrz_rec_daily_event' )) {
...
function zrz_rec_do_this_daily_rst() {
    global $wpdb;
    // 定时清除临时数据
    $wpdb->query( " DELETE FROM $wpdb->usermeta WHERE meta_key='zrz_rec_credit' OR meta_key='zrz_hot' OR meta_key='zrz_mission' OR meta_key='zrz_links_nub' OR meta_key='zrz_comment_total_up' OR meta_key='zrz_task' " );
