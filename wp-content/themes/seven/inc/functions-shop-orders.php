...
class Zrz_order_Message{
    ...
    public function get_order_count($type = '',$admin = false){
        ...
        $w = $type ? 'AND NOT order_state="w"' : 'WHERE NOT order_state="w"';
...
//检查临时订单
function zrz_check_order_state($pay_order_id){
    //检查临时订单
    global $wpdb;
    $table_name = $wpdb->prefix . 'zrz_order';
    ...
    $where = $wpdb->prepare( "order_state = %s AND order_id=%s", 'w',esc_sql($pay_order_id));

    $where = "SELECT * FROM $table_name WHERE $where";

    $check = $wpdb->get_results($where);
...
function zrz_notify_data_update($pay_order_id,$total_amount){
    ...
    if(strpos($pay_order_id,'pay_activity-') !== false){
        ...
        if($resout){
            ...
            apply_filters( 'zrz_activity_success', $arg );
        ...
        if($order_data['type'] == 'post'){
            ...
            foreach ($ids as $val) {
                ...
                if($resout){
                    ...
                    //增加钩子
                    do_action('zrz_shop_buy_action', $user_id,$val['id']);
                }
            }
