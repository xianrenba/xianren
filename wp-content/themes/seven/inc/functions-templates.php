            ...
            ?>
                ...
                <div :class="['user-login-menu','pos-a','pjt','transform-out',showMenu ? 'transform-in' : '']" v-cloak>
                    ...
                    <a href="<?php echo zrz_get_custom_page_link('notifications'); ?>" class="hd-avatar">
                        <?php echo __('消息','7b2'); ?><span class="fs12" v-text="msgCount" v-show="msgCount > 0"></span>
                    </a>
...
if ( ! function_exists( 'home_carousel_7b2' ) ) {
    function home_carousel_7b2(){
        ...
        $post_arr = get_option('zrz_swipe_posts');
        if(!is_array($post_arr) || empty($post_arr) || count($post_arr) < 4) {
            return;
        }
        $html = '<div id="carousel" class="home-wrapper mar10-b '.($is_mobile ? 'home-carousel' : '').'"><div ref="carousel" class="'.($is_mobile ? 'main-carousel' : '').'">';
        $count_post = count($post_arr);

        $i = 0;
        foreach ($post_arr as $id) {
            $i++;
            $post_type = get_post_type($id);
            $title = get_the_title($id);
            $link = get_permalink($id);
            $thumb = zrz_get_post_thumb($id);
            $cat = $post_type == 'post' ? zrz_get_first_category($id) : get_first_labs_7b2($id);
            ...
            if(($i == 1 && $count_post <= 4) || $is_mobile || ($i>=1 && $i <= $count_post-3 && $count_post > 4)){
                ...
                $html .= '<div class="wp-l '.($i == 1 && $count_post <= 4 ? 'fd pos-r' : '').' img-bg '.($is_mobile ? 'carousel-cell' : '').'" style="background-image:url('.zrz_get_thumb($thumb,570,365).')">
                        ...
                        <div class="wp-meta fs12 mar10-t">
                            '.get_avatar($author,40,40).zrz_get_user_page_link($author).ZRZ_THEME_DOT.zrz_time_ago($id).'
                        </div>
                    </div>' : '').'
                    </div>';
                ...
                $html .= '<div class="wp-r-b">
                    ...
                    <div class="img-bg pos-a" style="background-image:url('.zrz_get_thumb($thumb,280,176).')">
                        <a class="link-block" href="'.$link.'" rel="bookmark"></a>
                        '.($show ? '<div class="pos-a bg-bkg shadow">
                        <div class="wp-cat">'.$cat.'</div>
                        <h2>'.$title.'</h2>
...
if ( ! function_exists( 'home_mission_7b2' ) ) {
    function home_mission_7b2(){
        echo '<div class="box pd10 mission pos-r mar10-b mar10-t l1 hide" id="home-mission" ref="homeMission" v-cloak>
...
if ( ! function_exists( 'home_collection_7b2' ) ) {
    function home_collection_7b2(){
    	...
    	$html = '';
    	if ( !empty($taxonomies) ) {
    		$html = '<div id="home-collections" class="home-collections box mar10-b pos-r" @click="openList($event)">
    		<div class="pd10 b-b clearfix fs12 l1">
    		...
    		<ul class="l-8 pd5 " @click.stop="" v-show="locked">';
    		foreach ($taxonomies as $key => $val) {
    			if(in_array($val->term_id,$ids)){
    				$thumb = zrz_get_thumb($arr[$val->term_id],179,136);
    			}else{
    				$thumb = zrz_get_category_meta($val->term_id,'image',array(179,136));
    			}
    			$html .= '<li class="collections-item fd t-c l0">
    			<div class="collections-item-in pd10-b">
...
if ( ! function_exists( 'home_labs_7b2' ) ) {
    function home_labs_7b2(){
        ...
        echo '<div class="box mar10-b home-labs" ref="homeLabs">
