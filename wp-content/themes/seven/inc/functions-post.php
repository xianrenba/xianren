...
function zrz_get_share($type = false){
    ...
    $share = '<div class="share-box fs12">
...
function zrz_get_realted_posts($post_id){
    ...
    foreach ($post_ids as $id) {
        ...
        $html .= '<div class="fd post-related pos-r pd5">
			<div class="box">
...
function zrz_insert_post(){
	...
	//检查是否存在分类
	$cats = isset($_POST['cats']) && is_array($_POST['cats']) && !empty($_POST['cats']) ? $_POST['cats'] : '';
	if(!$cats){
	...
	//检查字数
	$str_length = strlen(wp_strip_all_tags($_POST['content']));
...
function zrz_inv_fn($atts, $content = null ){
	...
	if(count($codes) > 0){
		...
		<thead>
			<tr><td>编号</td><td>邀请码</td><td>奖励的积分</td><td>使用状态</td><td>使用者</td></tr>
		</thead>
		<tbody>';
		$i = 0;
...
function zrz_file_down($atts, $content = null ){
	...
	$pass = isset($atts['pass']) && !empty($atts['pass']) ? $atts['pass'] : '无';
	$code = isset($atts['code']) && !empty($atts['code']) ? $atts['code'] : '无';
	$local = isset($atts['local']) ? $atts['local'] : '';
	...
	return '<div class="single-file bg-blue-light pd20 mar20-b">
		<div class="single-file-title clearfix mar10-b">
			...
			<h2>'.$name.'</h2>
			<p>提取码：'.$pass.'，解压码：'.$code.'</p>
		</div>
		<div class="single-file-content pos-r">
			<a class="mar10-r button" target="_blank" href="'.$link.'" '.($local ? 'download="'.$name.basename( $link ).'"' : '').'><i class="zrz-icon-font-xiazai iconfont"></i>下载</a>
			<button class="down-qcode"><i class="zrz-icon-font-erweima1 iconfont"></i>二维码</button>
			<div class="down-qcode-img hide pos-a box"><img src="'.ZRZ_THEME_URI.'/inc/qrcode/index.php?c='.$link.'" /></div>
		</div>
	</div>';
}
	...
	*/
	...
	if($cap['key'] == 'credit'){
		...
		if(!is_user_logged_in()){
			return '<div class="content-hide-tips pos-r t-c">
			<i class="iconfont zrz-icon-font-suo-copy pos-a"></i>
				<div class="login-false mar20-b">当前隐藏内容需要支付'.zrz_coin(0,0,$credit).$name.'，请先登录</div>
			...
			<i class="iconfont zrz-icon-font-suo-copy pos-a"></i>
				<div class="no-credit">
				<div class="mar10-b t-c">阅读当前隐藏内容需要支付'.$name.' </div>'.zrz_coin(0,0,$credit).'
			...
			<i class="iconfont zrz-icon-font-suo-copy pos-a"></i>
			<div class="mar20-t">
			<p class="t-c">阅读当前隐藏内容需要支付'.$name.' '.zrz_coin(0,0,$credit).'</p>
	...
	*/
	...
	if($cap['key'] == 'rmb'){
		...
		if(!is_user_logged_in()){
			return '<div class="content-hide-tips pos-r t-c">
			<i class="iconfont zrz-icon-font-suo-copy pos-a"></i>
				<div class="login-false mar20-b">当前隐藏内容需要支付 <span class="content-hide-rmb">¥'.$need_rmb.'</span>，请先登录</div>
...
function zrz_is_id_in_swipe($post_id){
	$hd = get_option('zrz_swipe_posts');
	$hd = is_array($hd) ? $hd : array();
	if(in_array($post_id,$hd)) return true;
	return false;
}
...
function zrz_post_role_box($post){
	...
	$html = '
		...
		<div style="margin-bottom:20px">
			...
			<label style="display:block"><input type="radio" value="credit" name="capabilities" '.($capabilities == 'credit' ? 'checked' : '').' class="role-checked">积分阅读</label>
		...
		<div ="">
			<div id="zrz-credit" style="margin-top:20px;display:none;">
				<input type="text" placeholder="请输入积分" value="'.($capabilities == 'credit' ? $capabilities_value : '').'" name="credit">
			</div>
