...
<div class="wrap">
	...
	<form method="post">
        ...
        echo '<div id="qiniu-set" class="hide"><h3>七牛云设置</h3>';
        ...
        $option->table( array(
            ...
            array(
                ...
                'after'=>__('<p>可以是OSS的默认域名，也可以是自己绑定的域名。（支持https）</p>比如：<p><code>http://my-bucket.oss-cn-shenzhen.aliyuncs.com</code></p><p><code>http://www.my-domain.com</code></p>','ziranzhi2'),
                'key' => 'aliyun_host',
                'value' => $aliyun['host']
            ),
            array(
...
<script>
var zrzyun = jQuery('#media_place').find(':selected').val();
    if(zrzyun == 'qiniu'){
        jQuery('#qiniu-set').show();
    }else if(zrzyun == 'upyun'){
        jQuery('#upyun-set').show();
    }else if(zrzyun == 'aliyun'){
        jQuery('#aliyun-set').show();
    }else{
        jQuery('#local-set').show();
    }
console.log(zrzyun);
if(jQuery('#media_place'))
jQuery('#media_place').on('change', function() {
    var val = jQuery(this).find(':selected').val();
if(val === 'qiniu'){
    jQuery('#qiniu-set').show();
    jQuery('#upyun-set').hide();
    jQuery('#local-set').hide();
    jQuery('#aliyun-set').hide();
}else if(val === 'upyun'){
    jQuery('#upyun-set').show();
    jQuery('#local-set').hide();
    jQuery('#qiniu-set').hide();
    jQuery('#aliyun-set').hide();
}else if(val === 'aliyun'){
    jQuery('#aliyun-set').show();
    jQuery('#local-set').hide();
