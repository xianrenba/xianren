...
function utf8_excerpt_mb_warning() {
	$message = __( 'Your server does not seem to support the PHP <strong>mbstring</strong> library. The plugin can work, but will be extremely <strong>inefficient</strong>. It is strongly recommendated to contact your server provider and ask them to enable mbstring.', 'wp-utf8-excerpt');
...
if (!function_exists('utf8_excerpt')) {
	function utf8_excerpt ($text) {
		...
		//other cases, truncate according to characer count
		$text = strip_tags($text, $allowd_tag); 		
		$text = trim($text);
...
if (!function_exists('utf8_excerpt_readmore')) {
	function utf8_excerpt_readmore ($text) {
		...
		//if user sets %none%, do not display the "read more" link
		if ( '%none%' === $read_more_link ) {
			$text = force_balance_tags($text);
			return $text;
		}
		
		//otherwise add read_more_link
		$text .= "[......]";
		$text = force_balance_tags($text);
...
if (!function_exists('utf8_excerpt_for_excerpt')) {
	function utf8_excerpt_for_excerpt ($excerpt) {
		
		//in this case, the passed parameter $excerpt is the manual excerpt
...
<table class="form-table">
	...
	<tr valign="top">
		...
		<td><input type="text" name="read_more_link" value="<?php echo get_option('read_more_link') ? get_option('read_more_link') : READ_MORE_LINK ?>" style="width:400px" /> <span><?php _e("put <strong style='color:red'>%none%</strong> if you don't want a 'read more' link", "wp-utf8-excerpt"); ?></span></td>
	</tr>
