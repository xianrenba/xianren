...
$action =  isset($_REQUEST['act']) ? $_REQUEST['act'] : 'lostpassword';
...
case 'resetpass' :
	...
	if ( is_wp_error($user) ) {
		if ( $user->get_error_code() === 'expired_key' ){
			wp_redirect( _url_for('findpass') . '?act=lostpassword&error=expiredkey' );
		}
		else{
			wp_redirect( _url_for('findpass') . '?act=lostpassword&error=invalidkey' );
		}
		exit;
	}
	...
	if ( ( ! $errors->get_error_code() ) && isset( $_POST['pass1'] ) && !empty( $_POST['pass1'] ) ) {
		...
		wp_redirect( _url_for('findpass') . '?act=success' );
		exit;
	}
...
<div id="signin">	
	<div class="part resetpass">
		...
		<h1 class="steps"><?php echo $classactive1.$classactive2.$classactive3; ?></h1>

		<?php 
		if( $classactive1 ){ 
			if( $errors !== true ){
		?>
		<form action="<?php echo add_query_arg( 'act', 'lostpassword', _url_for('findpass') ); ?>" method="post">
			<?php errormsg($errors); ?>
			<p>
				<label class="icon" for="user_login">用户名/邮箱</label>
		...
		<form action="" method="post">
			<?php errormsg($errors); ?>
			<p>
				<label class="icon" for="pass1">设置新密码</label>
				<input class="input-control"  type="password" placeholder="请输入用户名" name="pass1" autofocus>
            </p>
            <p>
				<label class="icon" for="pass2">重复新密码</label>
				<input class="input-control"  type="password" placeholder="请输入用户名" name="pass2">
            </p>
			<p>
				<input class="submit" type="submit" value="确认提交" name="submit">
			</p>
		</form>
		<?php } ?>
	...
	<div class="foot-copyright">&copy; <?php echo date('Y'); ?> <a href="<?php echo home_url() ?>"><?php echo get_bloginfo('name') ?></a>|<?php if(get_option('users_can_register')==1){ ?><a href="<?php echo home_url('/m/signup'); ?>">注册</a>|<?php } ?><a href="<?php echo _url_for('signin'); ?>">登录</a></div>
</div>
<?php
...
function retrieve_password() {
	...
	$message .= network_site_url(_url_for('findpass') . "?act=resetpass&key=$key&login=" . rawurlencode($user_login), 'login');
