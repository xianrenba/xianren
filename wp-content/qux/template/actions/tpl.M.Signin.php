...
?>
    ...
    <form id="login" action="<?php echo site_url(); ?>/wp-login.php" method="post" novalidate="novalidate">
		<p class="status"></p>
        <p>
            <label class="icon" for="username">用户名/邮箱</label>
            <input class="input-control" id="username" type="text" placeholder="请输入用户名" name="username" required="" aria-required="true">
        </p>
        <p>
            <label class="icon" for="password">密码</label>
	...
	?>
	  ...
	  <a class="weixinlogin" id="weixinlogin"href="<?php echo $weibo_login ?>"><i class="fa fa-weixin"></i></a>
	  <?php } ?>
      </div>
    </div>
	<?php } ?>
    <?php if(get_option('users_can_register')==1){ ?><p class="signup-note">还没有账号? <a class="login-link" id="go-signup" href="<?php echo home_url('/m/signup'); ?>" rel="link">马上注册</a></p><?php } ?>
    </div>
