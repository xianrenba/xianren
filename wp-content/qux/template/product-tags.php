				...
				<a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>" rel="bookmark" class="fancyimg">
					<div class="thumb-img">
						...
						if ( has_post_thumbnail() ){
                             echo _get_post_thumbnail(375, 250, 'large');
					    }else{
                          	if( _hui('thumbnail_src') ){
    		                     $thumb_src = sprintf('<img data-src="%s" alt="%s" src="%s" class="lazy thumb">',um_timthumb(um_catch_first_image(),375,250), $post->post_title, get_stylesheet_directory_uri().'/img/thumbnail.png');
		                    }else{
    		                     $thumb_src = sprintf('<img src="%s" alt="%s" class="thumb">', um_timthumb(um_catch_first_image(),375,250), $post->post_title);
		                    }
                            echo $thumb_src;
						}
				        ?>
