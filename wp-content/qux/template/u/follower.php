...
<div class="page-wrapper">
   <div class="page-header">
		<h3 id="info"><?php _e('粉丝','um');?>(<?php echo um_follower_count($curauth->ID); ?>) <small><?php if($current_user->ID==$curauth->ID){_e('我的粉丝','um');}else{_e('TA的粉丝','um');} ?></small></h3>
   </div>
   <div class="widget-body">
		<div class="item">
			<ul class="flowlist followers-list clx">
				<?php if(um_follower_count($curauth->ID)==0){
                    if($current_user->ID==$curauth->ID){
                        echo '<div class="empty-content"><i class="fa fa-inbox"></i><p>还没有任何人关注您哦，多去转转吧！</p><a class="btn btn-info" href="/">去首页看看</a></div>';
                    }else{
						echo '<div class="empty-content"><i class="fa fa-inbox"></i><p>还没有任何人关注TA哦，赶快关注一下吧...</p><a class="btn btn-info" href="/">去首页看看</a></div>';
					}}else{ 
						// echo um_follow_list($curauth->ID,20); 
                        $all = um_follower_count($curauth->ID);
	                    $results = um_follower($curauth->ID,$number,$offset); 
		                ...
		                foreach($results as $result){
                            ...
                            <div class="box-inner transition">
                                ...
                                <?php echo um_get_avatar( $user->ID , '40' , um_get_avatar_type($user->ID) ); ?>
		                            <div class="mask">
