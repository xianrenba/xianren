		...
		if( $kefuorder ){
			...
			foreach ($kefuorder as $key => $value) {
				switch ($value) {
                    ...
                    case '8':
                         $layout = the_layout();
                        if( _hui('index_layout') ){
	                    if(is_home() && $layout=='index-blog'){
		                    $kefuhtml .= '<li id="layoutswt" class="mobile-hide"><a herf="' .get_bloginfo('url').'"><i class="fa fa-th is_blog" src="'.get_bloginfo('url').'"></i><span>CMS模式</span></a><h6>CMS模式<i></i></h6></li>'; 
		                     } elseif(is_home()&&$layout=='index-cms'){
	                    	$kefuhtml .= '<li id="layoutswt" class="mobile-hide"><a herf="'.get_bloginfo('url').'"><i class="fa fa-th-large is_cms" src="'.get_bloginfo('url').'"></i><span>卡片模式</span></a><h6>卡片模式<i></i></h6></li>'; 
		                     } elseif(is_home()){
		                    $kefuhtml .= '<li id="layoutswt" class="mobile-hide"><a herf="'.get_bloginfo('url'). '"><i class="fa fa-th-list is_blocks" src="'.get_bloginfo('url').'"></i><span>博客模式</span></a><h6>博客模式<i></i></h6></li>';
		                 }}
                    	 break;
