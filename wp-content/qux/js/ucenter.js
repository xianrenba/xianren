...
//支付宝
jQuery('body').on('click','#pay_action',function(){
  var _this = $(this),
      pid = _this.attr('data-pid'),
      order = _this.attr('data-id');
  if((jsui.alipay_url.indexOf("xhpay") != -1 )){
  	    $.ajax({
  	    	type: 'POST',
  	    	dataType: 'json',
  	    	url: jsui.alipay_url, 
...
//微信
jQuery('body').on('click','#wxpay_action',function(){
  var _this = $(this),
      pid = _this.attr('data-pid'),
      order = _this.attr('data-id');
  	  $.ajax({
		...
		success: function(response) {
			...
			if(response.success==0){
                ...
                $('.pay-content-warp').html('<div class="price">￥'+response.order_price+'</div><div class="payqrcode"><img src="'+response.code_url+'"></div><div class="pay-tips">请使用微信扫一扫<br>扫描二维码支付</div>');
                var time = 20;
      ...
      //swal( "操作失败 ", "微信正在集成中...请选择支付宝付款！","error");
});
...
$(document).ready(function(){
	...
	$("#cover-sure").bind('click',function(A){
		...
		F=F!==''?F.replace('-small','-full'):default_cover;
		$.ajax({
			type:"POST",
			dataType:'json',
			url:um.ajax_url,
	...
	if($('.um_sign .part').outerHeight()%2 != 0){
		$('.um_sign .part').css('height',$('.um_sign .part').outerHeight()+1);
	}
	...
	jQuery('a#upload-cover').on('click',function(e){  
		...
		upload_frame.on('select',function(){   
			attachment = upload_frame.state().get('selection').first().toJSON();   
			//jQuery('input[name='+value_id+']').val(attachment.url); 
            B.attr('src',um.timthumb+attachment.url+"&w=240&h=64&zc=1&q=100");C.attr('src',um.timthumb+attachment.url+"&w=1200&h=300&zc=1&q=100");
            var F = attachment.url;//imgurl;//um.timthumb+imgurl+"&w=1200&h=300&zc=1&q=100";
			var E = $("#cover-sure").attr("curuserid");
			$.ajax({
				type:"POST",
				dataType:'json',
