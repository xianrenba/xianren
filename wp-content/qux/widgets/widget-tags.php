...
class widget_ui_tags extends WP_Widget {
	...
	function __construct(){
		parent::__construct( 'widget_ui_tags', 'qux 标签云', array( 'classname' => 'widget_ui_tags' ) );
	}
	...
	function widget( $args, $instance ) {
		...
		$style=' class="items"';
		if( $color ) $style = ' class="items color"';
		echo $before_widget;
...
?>
		...
		<p>
			<label>
				<input style="vertical-align:-3px;margin-right:4px;" class="checkbox" type="checkbox" <?php checked( $instance['color'], 'on' ); ?> id="<?php echo $this->get_field_id('color'); ?>" name="<?php echo $this->get_field_name('color'); ?>">彩色标签
			</label>
		</p>
