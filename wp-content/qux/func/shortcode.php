...
function xcollapse($atts, $content = null) {
    ...
    return '<div class="collapse-wrap" style="margin: 0.5em 0;"><div class="xControl  collapseButton"><i class="fa fa-angle-right"></i>'.$title.'</div><div class="xContent" >'.$content.'</div></div>';
}
...
function product_shortcode($atts, $content = null){
	...
	if(!empty($id)) {
	    ...
	    return '<div class="embed-product">
        <img src="'.um_timthumb($imgsrc,100,100).'">
        <div class="product-info">
        <h4><a href="'.$href.'">'.$title.'</a></h4>
        <div class="price">'.$preice.'<span>'.$money.'</span></div>
        <div class="post-meta">&nbsp;数量：<span>'.$amount.'</span>&nbsp;&nbsp;&nbsp;销量：<span>'.$sales.'</span></div>
        <a class="btn btn-success btn-buy" href="'.$href.'"><i class="fa fa-shopping-cart"></i>立即购买</a>
        </div></div>';	
	}else{
...
add_action( 'init', 'theme_init_smilies', 5 );
...
function get_author_class($comment_author_email,$user_id){  
    global $wpdb;  
    $author_count = count($wpdb->get_results("SELECT comment_ID as author_count FROM $wpdb->comments WHERE comment_author_email = '$comment_author_email' and user_id = '$user_id' "));
    if(!empty(get_userdata($user_id)->roles) && in_array('administrator',get_userdata($user_id)->roles)) { $admin = true; }else{$admin = false;}
    //$adminEmail = get_option('admin_email');
    if(/*$comment_author_email == $adminEmail ||*/ $admin) return;  
