...
//获取开通会员订单记录2
function _getUserMemberOrders($uid='',$limit = 20, $offset = 0){
	global $wpdb;
	if(empty($uid)):
	$user_info = wp_get_current_user();
	$id = $user_info->ID;
	...
	$prefix = $wpdb->prefix;
	$table = $prefix.'um_orders';
	$vip_orders=$wpdb->get_Results("select * from ".$table." where user_id=".$id." and product_id in (-1,-2,-3,-4) ORDER BY id DESC LIMIT $offset,$limit",'ARRAY_A');
	return $vip_orders;
}
