...
function um_follow_button($uid){
	...
	if($check){
		if($check->follow_status==2){$button = '<small data-uid="'.$uid.'" data-act="disfollow" class="follow-btn followed"><i class="fa fa-exchange"></i>互相关注</small>';}else{$button = '<small data-uid="'.$uid.'" data-act="disfollow" class="follow-btn followed"><i class="fa fa-check"></i>已关注</small>';}
	}else{$button = '<small data-uid="'.$uid.'" data-act="follow" class="follow-btn unfollowed"><i class="fa fa-plus"></i>关 注</small>';}
	return $button;
}
