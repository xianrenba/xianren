...
function product_smallest_price($product_id){
	...
	if($vip_discount<1&&$vip_discount>=0){$vip_price = $currency == 'cash' ? $original_price*$vip_discount : intval($original_price*$vip_discount);}else{$vip_price = $original_price;}
...
function payment_cho($oid, $pid ,$pice){
	...
	ob_start(); ?>
		...
		<div class="order-pay-content">
			...
			<div class="preice-title"><h2>付款金额</h2> <div class="pay-price"><span>¥</span><?php echo $pice; ?></div></div>
			<div class="orderdetail-status">
				...
				if((_hui('alipay_option') == 'alipay' && _hui('alipay_id') && _hui('alipay_key'))||
					...
					(_hui('alipay_option') == 'xhpay' && _hui('xhpay_appid') && _hui('xhpay_secret'))){ 
						echo '<button class="btn btn-primary" id="pay_action" data-pid="'.$pid.'" data-id="'.$oid.'">支付宝付款</button>';
					}
				if((_hui('wxpay_option') == 'wxpay' && _hui('wechat_mchid') && _hui('wechat_appid'))||
				   ...
				   (_hui('wxpay_option') == 'xhpay' && _hui('xhpay_appid') && _hui('xhpay_secret'))){ 
				       echo '<button class="btn btn-success" id="wxpay_action" data-pid="'.$pid.'" data-id="'.$oid.'">微信付款</button>';
				   }
				?>
			</div>
            </div>
	...
	if($quantity < 1){
		...
		if($currency==0){
			...
			if($price[5]==0){				
			    ...
			    if($insert){
                   $content = payment_cho($insert, $pid ,$price[5]);
                   if($content){
                       $redirect = 1;
                       $success = 1;
				       $order_id = $insert;
...
function continue_the_order(){
	...
	if($order){
		...
		elseif($order->order_currency=='cash'&&$order->order_status==1):
            $content = payment_cho($order->order_id, $order->product_id ,$order->order_price);
            if($content){
                $redirect = 1;
                $success = 1;
            }else{
...
function check_order(){
	...
	if($result){
		$success = 1;
        $redirect = add_query_arg('tab', 'orders', esc_url( get_author_posts_url( $current_user->ID ) )).'&order='.$result->id;
	}
