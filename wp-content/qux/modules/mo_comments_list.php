...
function mo_comments_list($comment, $args, $depth) {
    ...
    echo '<div class="comt-main" id="div-comment-'.get_comment_ID().'">';
        ...
        //显示VIP用户
        if(getUserMemberType($comment->user_id) !==false) echo um_member_icon($comment->user_id);
		//添加用户等级
		get_author_class($comment->comment_author_email ,$comment->user_id);
		//添加博主认证
        if(!empty(get_userdata($comment->user_id)->roles) && in_array('administrator',get_userdata($comment->user_id)->roles)) { echo "<a title='博主认证' class='vip'></a>"; }
